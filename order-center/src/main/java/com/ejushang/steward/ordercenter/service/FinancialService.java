...
//        再设置与订单项有关的字段
    ...
    private void renderMerger2OrderItem2Excel(Row itemRow, int startCellIndex, FinancialOrderVo financialOrderVo) {
        ...
        PoiUtil.createCell(itemRow, startCellIndex++, EJSDateUtils.formatDate(financialOrderVo.getHappenTime(),EJSDateUtils.DateFormatType.DATE_FORMAT_STR));
        PoiUtil.createCell(itemRow, startCellIndex++, EJSDateUtils.formatDate(financialOrderVo.getBuyTime(),EJSDateUtils.DateFormatType.DATE_FORMAT_STR));
        PoiUtil.createCell(itemRow, startCellIndex++, EJSDateUtils.formatDate(financialOrderVo.getPayTime(),EJSDateUtils.DateFormatType.DATE_FORMAT_STR));
        if (financialOrderVo.getPrintTime() != null) {
            PoiUtil.createCell(itemRow, startCellIndex++, EJSDateUtils.formatDate(financialOrderVo.getPrintTime(),EJSDateUtils.DateFormatType.DATE_FORMAT_STR));  //打印时间
        } else {
            startCellIndex++;
        }
